...
class DynamicThreshRegistry(object):
    ...
    def get(self, tag=None):
        ...
        if tag in self._tag_metrics.keys():
            ...
            metric = build_thresholding_method(
                tag=tag, dataset=self.dataset, config=self.model_cfg)
...
class PVRCNN_SSL(Detector3DTemplate):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        self.pv_rcnn = PVRCNN(model_cfg=model_cfg,
                              num_class=num_class, dataset=dataset)

        self.pv_rcnn_ema = PVRCNN(
            model_cfg=model_cfg_copy, num_class=num_class, dataset=dataset_copy)
        ...
        self.thresh_registry = DynamicThreshRegistry(
            dataset=self.dataset, model_cfg=model_cfg)
        for bank_configs in model_cfg.get("FEATURE_BANK_LIST", []):
            feature_bank_registry.register(
                tag=bank_configs["NAME"], **bank_configs)

        for metrics_configs in model_cfg.get("METRICS_BANK_LIST", []):
            metrics_registry.register(
                tag=metrics_configs["NAME"], dataset=self.dataset, **metrics_configs)
    ...
    def _prep_bank_inputs(self, batch_dict, inds, num_points_threshold=20):
        ...
        with torch.no_grad():
            batch_gt_feats = self.pv_rcnn.roi_head.pool_features(
                selected_batch_dict, use_gtboxes=True)

        batch_gt_feats = batch_gt_feats.view(
            *batch_dict['gt_boxes'].shape[:2], -1)
        ...
        for ix in inds:
            ...
            smpl_id = torch.from_numpy(batch_dict['frame_id'].astype(np.int32))[
                ix].to(gt_boxes.device)
            ...
            num_points_in_gt = roiaware_pool3d_utils.points_in_boxes_cpu(
                points.cpu(), gt_boxes.cpu()).sum(dim=-1)
            ...
            if valid_gts_mask.sum() == 0:
                print(
                    f"no valid gt instances with enough points in frame {batch_dict['frame_id'][ix]}")
                continue
    ...
    def _rectify_pl_scores(self, batch_dict_ema, unlabeled_inds):
        ...
        pred_wa = torch.sigmoid(
            batch_dict_ema['batch_cls_preds']).detach().clone()
        ...
        if thresh_reg.iteration_count > 0:
            ...
            pred_wa_ulb_aligned = pred_wa_ulb * thresh_reg.ema_pred_wa_lab / \
                (thresh_reg.ema_pred_wa_ulb + 1e-6)
    ...
    def _gen_pseudo_labels(self, batch_dict_ema, ulb_inds):
        ...
        pseudo_labels, _ = self.pv_rcnn_ema.post_processing(
            batch_dict_ema, no_recall_dict=True)
    ...
    def _prep_batch_dict(batch_dict):
        ...
        batch_dict['ori_unlabeled_boxes'] = batch_dict['gt_boxes'][unlabeled_inds, ...].clone(
        ).detach()
    ...
    def _forward_training(self, batch_dict):
        ...
        pseudo_boxes, pseudo_scores, pseudo_sem_scores = self._filter_pseudo_labels(
            pseudo_labels, ulb_inds)

        self._fill_with_pseudo_labels(
            batch_dict, pseudo_boxes, ulb_inds, lbl_inds)

        # apply student's augs on teacher's pseudo-labels (filtered) only (not points)
        batch_dict = self.apply_augmentation(
            batch_dict, batch_dict, ulb_inds, key='gt_boxes')

        for cur_module in self.pv_rcnn.module_list:
            batch_dict = cur_module(batch_dict)

        # 1. @Student - Get shared_features over GTs
        batch_dict = self.pv_rcnn.roi_head.forward(
            batch_dict, test_only=True, use_gtboxes=True)  # strong

        # 2. @Student - Get shared_features over flipped_gts (BATCH_DICT_PAIR)
        with torch.no_grad():
            for cur_module in self.pv_rcnn.module_list:
                try:
                    batch_dict_pair = cur_module(
                        batch_dict_pair, test_only=True, use_gtboxes=True)  # weak
                except TypeError as e:
                    batch_dict_pair = cur_module(batch_dict_pair)

        if self.model_cfg.ROI_HEAD.ADAPTIVE_THRESH_CONFIG.get('ENABLE', False):
            pred_strong_aug_before_nms_org = torch.sigmoid(
                batch_dict['batch_cls_preds']).detach().clone()
            pred_dicts_std, recall_dicts_std = self.pv_rcnn_ema.post_processing(
                batch_dict, no_recall_dict=True)

            metrics_input = defaultdict(list)
            for ind in range(len(pred_dicts_std)):
                batch_type = 'unlab' if ind in ulb_inds else 'lab'
                metrics_input[f'pred_weak_aug_{batch_type}_before_nms'].append(
                    batch_dict_ema['batch_cls_preds_org'][ind])
                metrics_input[f'pred_weak_aug_{batch_type}_after_nms'].append(
                    pseudo_labels[ind]['pred_scores'].clone().detach())
                metrics_input[f'pred_strong_aug_{batch_type}_before_nms'].append(
                    pred_strong_aug_before_nms_org[ind])
                metrics_input[f'pred_strong_aug_{batch_type}_after_nms'].append(
                    pred_dicts_std[ind]['pred_scores'].clone().detach())
            self.thresh_registry.get(
                tag='pl_adaptive_thresh').update(**metrics_input)

        # Update the bank with student's features from augmented labeled data
        bank = feature_bank_registry.get('gt_aug_lbl_prototypes')
        sa_gt_lbl_inputs = self._prep_bank_inputs(
            batch_dict, lbl_inds, bank.num_points_thresh)
        ...
        loss_rcnn_cls, loss_rcnn_box, ulb_loss_cls_dist, tb_dict = self.pv_rcnn.roi_head.get_loss(
            tb_dict, scalar=False)
        ...
        loss += reduce_loss_fn(loss_rpn_box[lbl_inds, ...]) + reduce_loss_fn(
            loss_rpn_box[ulb_inds, ...]) * self.unlabeled_weight
        ...
        if self.unlabeled_supervise_cls:
            loss += reduce_loss_fn(loss_rpn_cls[ulb_inds, ...]
                                   ) * self.unlabeled_weight
        if self.model_cfg['ROI_HEAD'].get('ENABLE_SOFT_TEACHER', False) or self.model_cfg.get('UNLABELED_SUPERVISE_OBJ', False):
            loss += reduce_loss_fn(loss_rcnn_cls[ulb_inds, ...]
                                   ) * self.unlabeled_weight
        if self.unlabeled_supervise_refine:
            loss += reduce_loss_fn(loss_rcnn_box[ulb_inds, ...]
                                   ) * self.unlabeled_weight
        ...
        if self.model_cfg['ROI_HEAD'].get('ENABLE_PROTO_CONTRASTIVE_LOSS', False):
            proto_cont_loss = self._get_proto_contrastive_loss(
                batch_dict, bank, ulb_inds)
            if proto_cont_loss is not None:
                loss += proto_cont_loss * \
                    self.model_cfg['ROI_HEAD']['PROTO_CONTRASTIVE_LOSS_WEIGHT']
                tb_dict['proto_cont_loss'] = proto_cont_loss.item()
        if self.model_cfg['ROI_HEAD'].get('ENABLE_INSTANCE_SUP_LOSS', False):
            lbl_inst_cont_loss = self._get_instance_contrastive_loss(
                batch_dict, batch_dict_pair, lbl_inds)
            if lbl_inst_cont_loss is not None:
                loss += lbl_inst_cont_loss * \
                    self.model_cfg['ROI_HEAD']['INSTANCE_CONTRASTIVE_LOSS_WEIGHT']
                tb_dict['proto_cont_loss'] = lbl_inst_cont_loss.item()

        tb_dict_ = self._prep_tb_dict(
            tb_dict, lbl_inds, ulb_inds, reduce_loss_fn)
        ...
        for tag in self.thresh_registry.tags():
            if results := self.thresh_registry.get(tag).compute():
                ...
                tb_dict_.update(
                    {tag + key: val for key, val in results.items()})
    ...
    def _get_proto_contrastive_loss(self, batch_dict, bank, ulb_inds):
        ...
        sa_pl_feats = self.pv_rcnn.roi_head.pool_features(
            batch_dict, use_gtboxes=True).view(B * N, -1)
        pl_labels = batch_dict['gt_boxes'][..., -1].view(-1).long() - 1
        proto_cont_loss = bank.get_proto_contrastive_loss(
            sa_pl_feats, pl_labels)
        ...
        if ulb_nonzero_mask.sum() == 0:
            print(
                f"No pl instances predicted for strongly augmented frame(s) {batch_dict['frame_id'][ulb_inds]}")
            return
    ...
    def _get_instance_contrastive_loss(self, batch_dict, batch_dict_pair, lbl_inds, temperature=1.0, base_temperature=1.0):
        ...
        shared_ft = batch_dict['shared_features_gt'].view(
            batch_dict['batch_size'], -1, 256)[lbl_inds]
        shared_ft_pair = batch_dict_pair['shared_features_gt'].view(
            batch_dict['batch_size'], -1, 256)[lbl_inds]
        ...
        meta_data = {'to_mask': ''}
        valid_instances = np.intersect1d(
            instance_idx.cpu().numpy(), instance_idx_pair.cpu().numpy())
        ...
        intersect_mask = torch.tensor(
            [idx in valid_instances for idx in instance_idx], device=device, dtype=torch.bool)
        intersect_mask_pair = torch.tensor(
            [idx in valid_instances for idx in instance_idx_pair], device=device, dtype=torch.bool)
        ...
        mask = torch.eq(labels, labels.T).float().to(
            device)  # (B*N, B*N)  # 48,48

        combined_shared_features = torch.cat([shared_ft.unsqueeze(
            1), shared_ft_pair.unsqueeze(1)], dim=1)  # B*N,num_pairs,channel_dim
        ...
        contrast_feature = torch.cat(torch.unbind(
            combined_shared_features, dim=1), dim=0)
        contrast_feature = F.normalize(
            contrast_feature.view(-1, combined_shared_features.shape[-1]))
        # compute logits
        anchor_dot_contrast = torch.div(torch.matmul(
            contrast_feature, contrast_feature.T), temperature)
        ...
        # mask-out self-contrast cases
        self_contrastive_mask = torch.scatter(torch.ones_like(mask), 1, torch.arange(
            batch_size_labeled * num_pairs).view(-1, 1).to(device), 0)
        mask = mask * self_contrastive_mask  # mask * logits_mask

        exp_logits = torch.exp(logits) * \
            self_contrastive_mask  # compute log_prob
    ...
    def _prep_tb_dict(tb_dict, lbl_inds, ulb_inds, reduce_loss_fn):
        ...
        for key in tb_dict.keys():
            if key == 'proto_cont_loss':
                ...
                tb_dict_[f"{key}_labeled"] = reduce_loss_fn(
                    tb_dict[key][lbl_inds, ...])
                tb_dict_[f"{key}_unlabeled"] = reduce_loss_fn(
                    tb_dict[key][ulb_inds, ...])
            else:
                tb_dict_[key] = tb_dict[key]
    ...
    def _add_teacher_scores(self, batch_dict, batch_dict_ema, ulb_inds):
        ...
        batch_dict_std = self.reverse_augmentation(
            batch_dict_std, batch_dict, ulb_inds)

        # Perturb Student's ROIs before using them for Teacher's ROI head
        if self.model_cfg.ROI_HEAD.ROI_AUG.get('ENABLE', False):
            augment_rois = getattr(
                augmentor_utils, self.model_cfg.ROI_HEAD.ROI_AUG.AUG_TYPE, augmentor_utils.roi_aug_ros)
            # rois_before_aug is used only for debugging, can be removed later
            batch_dict_std['rois_before_aug'] = batch_dict_std['rois'].clone(
            ).detach()
            batch_dict_std['rois'][ulb_inds] = augment_rois(
                batch_dict_std['rois'][ulb_inds], self.model_cfg.ROI_HEAD)

        self.pv_rcnn_ema.roi_head.forward(batch_dict_std, test_only=True)
        batch_dict_std = self.apply_augmentation(
            batch_dict_std, batch_dict, ulb_inds, key='batch_box_preds')
        pred_dicts_std, recall_dicts_std = self.pv_rcnn_ema.post_processing(batch_dict_std,
                                                                            no_recall_dict=True,
                                                                            no_nms_for_unlabeled=True)
        rcnn_cls_score_teacher = - \
            torch.ones_like(
                self.pv_rcnn.roi_head.forward_ret_dict['rcnn_cls_labels'])
        batch_box_preds_teacher = torch.zeros_like(
            self.pv_rcnn.roi_head.forward_ret_dict['batch_box_preds'])
    ...
    def dump_statistics(self, batch_dict, unlabeled_inds):
        ...
        batch_roi_labels = [roi_labels.clone().detach()
                            for roi_labels in batch_roi_labels]
        ...
        batch_ori_gt_boxes = [ori_gt_boxes.clone().detach()
                              for ori_gt_boxes in batch_ori_gt_boxes]

        for i in range(len(batch_rois)):
            valid_rois_mask = torch.logical_not(
                torch.all(batch_rois[i] == 0, dim=-1))
            ...
            valid_gt_boxes_mask = torch.logical_not(
                torch.all(batch_ori_gt_boxes[i] == 0, dim=-1))
            ...
            if num_gts > 0 and num_preds > 0:
                ...
                overlap = iou3d_nms_utils.boxes_iou3d_gpu(
                    valid_rois[:, 0:7], valid_gt_boxes[:, 0:7])
                preds_iou_max, assigned_gt_inds = overlap.max(dim=1)
                self.val_dict['iou_roi_gt'].extend(preds_iou_max.tolist())

                cur_iou_roi_pl = self.pv_rcnn.roi_head.forward_ret_dict[
                    'gt_iou_of_rois'][cur_unlabeled_ind]
                self.val_dict['iou_roi_pl'].extend(cur_iou_roi_pl.tolist())

                cur_pred_score = torch.sigmoid(
                    batch_dict['batch_cls_preds'][cur_unlabeled_ind]).squeeze()
                ...
                if 'rcnn_cls_score_teacher' in self.pv_rcnn.roi_head.forward_ret_dict:
                    cur_teacher_pred_score = self.pv_rcnn.roi_head.forward_ret_dict['rcnn_cls_score_teacher'][
                        cur_unlabeled_ind]
                    self.val_dict['teacher_pred_scores'].extend(
                        cur_teacher_pred_score.tolist())

                    cur_weight = self.pv_rcnn.roi_head.forward_ret_dict[
                        'rcnn_cls_weights'][cur_unlabeled_ind]
                    self.val_dict['weights'].extend(cur_weight.tolist())

                cur_roi_score = torch.sigmoid(
                    self.pv_rcnn.roi_head.forward_ret_dict['roi_scores'][cur_unlabeled_ind])
                ...
                cur_num_points_roi = self.pv_rcnn.roi_head.forward_ret_dict[
                    'num_points_in_roi'][cur_unlabeled_ind]
                self.val_dict['num_points_in_roi'].extend(
                    cur_num_points_roi.tolist())
                ...
                cur_iteration = torch.ones_like(
                    preds_iou_max) * (batch_dict['cur_iteration'])
        ...
        output_dir = os.path.split(
            os.path.abspath(batch_dict['ckpt_save_dir']))[0]
    ...
    def _filter_pseudo_labels(self, pred_dicts, unlabeled_inds):
        ...
        for pseudo_box, pseudo_label, pseudo_score, pseudo_sem_score, pseudo_box_var, pseudo_score_var in zip(
            ...
            if pseudo_label[0] == 0:
                pseudo_boxes.append(
                    torch.cat([pseudo_box, pseudo_label.view(-1, 1).float()], dim=1))
                pseudo_sem_scores.append(pseudo_sem_score)
                pseudo_scores.append(pseudo_score)
                continue
            ...
            if self.model_cfg.ROI_HEAD.ADAPTIVE_THRESH_CONFIG.get('ENABLE', False):
                ...
                if thresh_reg.relative_ema_threshold is not None:
                    pl_thresh = [
                        thresh_reg.relative_ema_threshold.item()] * len(self.thresh)
            ...
            valid_inds = valid_inds & (
                pseudo_sem_score > sem_conf_thresh.squeeze())
    ...
    def _fill_with_pseudo_labels(self, batch_dict, pseudo_boxes, unlabeled_inds, labeled_inds, key=None):
        ...
        if max_box_num >= max_pseudo_box_num:
            for i, pseudo_box in enumerate(pseudo_boxes):
                ...
                if diff > 0:
                    pseudo_box = torch.cat([pseudo_box, torch.zeros(
                        (diff, 8), device=pseudo_box.device)], dim=0)
            ...
            new_ins_idx = torch.full(
                (ori_boxes.shape[0], max_pseudo_box_num), fill_value=-1, device=ori_boxes.device)
            for idx in labeled_inds:
                diff = max_pseudo_box_num - ori_boxes[idx].shape[0]
                new_box = torch.cat([ori_boxes[idx], torch.zeros(
                    (diff, 8), device=ori_boxes[idx].device)], dim=0)
                new_boxes[idx] = new_box
                new_ins_idx[idx] = torch.cat(
                    [ori_ins_ids[idx], -torch.ones((diff,), device=ori_boxes[idx].device)], dim=0)
            for i, pseudo_box in enumerate(pseudo_boxes):
