#https://github.dev/chaytonmin/Occupancy-MAE
...
class Voxel_MAE(nn.Module):
    ...
    def __init__(self, model_cfg, input_channels, grid_size, voxel_size, point_cloud_range, **kwargs):
        ...
        for k in range(0, self.model_cfg.SHARED_FC.__len__()):
            shared_fc_list.extend([
                nn.Conv1d(100, self.model_cfg.SHARED_FC[k], kernel_size=1, bias=False),
                nn.BatchNorm1d(self.model_cfg.SHARED_FC[k]),
                nn.ReLU()
            ])

            if k != self.model_cfg.SHARED_FC.__len__() - 1 :
                shared_fc_list.append(nn.Dropout(p=0.5))
    ...
    def forward(self, batch_dict):
        ...
        # Applying I-JEPA on voxelbackbone
        # TODO Advait:  Adding a new MLP here and passing x_up3 would make it similar to I-JEPA ? 
        # projection = self.shared_fc_layer(x_up_3)
        # if student:
            #batch_dict['projection'] = projection
        batch_dict['reconstruction_target_mae'] = input_sp_tensor_ones.dense()
        batch_dict['predicted_reconstructed'] = x_up3
